...
ResizeImage = {
    resizeImage:function(file, no, element) {
        ...
        EXIF.getData(file, function() {
            ...
            var mpImg = new MegaPixImage(file);        
            mpImg.render(element, { maxWidth: 320, orientation: orientation }, function() {
    ...
    ,adaptImage:function (image, size, x, y, canvas) {
        var workspace,bitmap;
        var rect = new createjs.Rectangle(x, y, size, size);
...
TouchPhoto = {
    ...
    ,calculatePosition:function (width, height)
    {
        var ret = {x:0,y:0};
        var t = TouchPhoto.transform;
        ret.x = (t.translate.x-((width*t.scale-width)/2))*1/t.scale;
        ret.y = (t.translate.y-((height*t.scale-height)/2))*1/t.scale;
        return ret;
    }
};